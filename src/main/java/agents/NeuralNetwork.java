...
public class NeuralNetwork implements Serializable {
	...
	public static final int[] LAYER_SIZES = { NeuralFactors.in.NUM_FACTORS, 4,
			ActionManager.getNumActions() + 1 /* speed */ };
		...
		for (int weightLayer = 0; weightLayer < NUM_WEIGHTS; ++weightLayer) {
			for (int nodeNextLayer = 0; nodeNextLayer < LAYER_SIZES[weightLayer + 1]; ++nodeNextLayer) {
				...
				for (int nodeCurrentLayer = 0; nodeCurrentLayer < LAYER_SIZES[weightLayer]; ++nodeCurrentLayer) {
					z[weightLayer + 1][nodeNextLayer] += (z[weightLayer][nodeCurrentLayer]
	...
	public int neuralMagic(final ArrayList<Action> actions) {
		...
		int selection = -1;
		for (int i = 0; i < ActionManager.getNumActions(); ++i) {
	...
	public float getSpeed() {
		...
		return z[LAYER_SIZES.length - 1][ActionManager.getNumActions()];
	}
}
