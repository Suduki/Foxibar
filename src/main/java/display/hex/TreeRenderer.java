...
public class TreeRenderer {
	...
	private class TreeTopRenderer extends TubeRenderer {
		...
		protected float heightToRadius(float h, float tubeMaxRadius) {
			if (h < (1f - plantBeingRendered.health)) {
				return 0;
			}
			return (h * (1f - h)) * 4 * tubeMaxRadius;
		}

		private Plant plantBeingRendered;

		public void setPlantBeingRendered(Plant plantBeingRendered) {
			this.plantBeingRendered = plantBeingRendered;
		}
	}
	...
	void renderTreeAt(Plant plant, Vector3f pos, float x, float z) {
		...
		if (health > 0.05f) {
			...
			topRenderer.setPlantBeingRendered(plant);
			topRenderer.renderTube(pos, treeTopHeight, treeTopWidth, yStart, windForce, 1f - health);
		}
