...
public class Vision {
	...
	public static float calculateCircularDistance(int pos, int pos2) {
		if (!WARNING_PRINTED) {
	...
	public static void updateAnimalZone(Animal animal) {
		int oldPos = animal.oldPos;
		int pos = animal.pos;
		...
		if (oldZoneX != zoneX || oldZoneY != zoneY) {
			removeAnimalFromZone(animal, oldZoneX, oldZoneY);
			addAnimalToZone(animal, zoneX, zoneY);
		}
	}
	
	public static void addAnimalToZone(Animal animal) {
		int zoneX = getZoneXFromPos(animal.pos);
		int zoneY = getZoneYFromPos(animal.pos);
		addAnimalToZone(animal, zoneX, zoneY);
	}
	public static void removeAnimalFromZone(Animal animal) {
		int zoneX = getZoneXFromPos(animal.pos);
		int zoneY = getZoneYFromPos(animal.pos);
		removeAnimalFromZone(animal, zoneX, zoneY);
	}
	
	private static void addAnimalToZone(Animal animal, int zoneX, int zoneY) {
		zoneGrid[zoneX][zoneY].animalsInZone.add(animal);
	}
	private static void removeAnimalFromZone(Animal animal, int zoneX, int zoneY) {
		zoneGrid[zoneX][zoneY].animalsInZone.remove(zoneGrid[zoneX][zoneY].animalsInZone.indexOf(animal));
	}
	...
	private static class Zone {
		...
		public Zone() {
			animalsInZone = new ArrayList<Animal>();
			color = new float[3];
