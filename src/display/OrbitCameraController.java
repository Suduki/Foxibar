...
class OrbitCameraController implements InputHandlerI {
	...
	OrbitCameraController(Camera pCamera) {
		mCamera = pCamera;
		mDistance = 0.5f;//mCamera.getEyePosition().distance(mCamera.getTargetPosition());
	}
	...
	private void update() {
		...
		mCamera.setEyePosition(new Vector3f((float)x, (float)y, (float)z));
	}
